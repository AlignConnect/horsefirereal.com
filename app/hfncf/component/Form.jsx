...
const Form = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefirereal.com/HFNCF")
            formdata.append("ip", check ? check : "")
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked)
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFNCF ?? "horsefirereal.com/hfncf")
                        ...
                        {CustomerDataValue?.map((data, key) => {
                            return <div className="flex flex-col my-6" key={key}>
                                ...
                                <div className="relative">
                                    <input {...data.inputValue} className={`p-1 py-3 border border-black w-full rounded-lg focus:outline-none focus:ring-0 focus:ring-red-100 ${data.icon ? 'pl-[3.5rem]' : ''}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
