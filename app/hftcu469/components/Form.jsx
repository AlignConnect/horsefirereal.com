...
const Form = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefirereal.com/HFTCU469");
            formdata.append("ip", check ? check : "");
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked);
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFTCU469 ?? "horsefirereal.com/hftcu469");
            ...
            <section className='relative'>
                ...
                <div className="relative z-20 pt-0 pt-5 pb-10 sm:py-10 md:px-2" id="form">
                    ...
                    <div className="text-center py-5">
                        <h1 className="text-[1.5rem] md:text-3xl font-bold  text-yellow-500">कॉल पर विशेषज्ञ डॉक्टरों से<br /> मुफ्त सलाह के लिए फॉर्म भरें!</h1>
                    </div>
                    <div className="text-white " id='formcallhide' >
                        <div className="bg-[#191d1e] max-w-2xl mx-2 md:mx-12 md:px-4 px-1 rounded-md" style={{ boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px" }}>
                            <div className="py-5">
                                    ...
                                    {CustomerDataValue?.map((data, key) => {
                                        return <div className="flex flex-col" key={key}>
                                            ...
                                            <div className="relative z-20">
                                                <input {...data.inputValue} className={`px-3 py-3 border border-black w-full rounded-lg bg-[#303435]  ${data.icon ? 'pl-[4.5rem]' : ''}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
                                                {data.icon && <span className="absolute fontPoppins ransform translate-x-[-50%] translate-y-[-50%] top-1/2 left-7 border-r border-white px-2">{data.icon}</span>}
                                            </div>
                                        </div>
                                    })}

                                    <div className="flex justify-center items-center pt-6 ">
                                        <button onClick={handleFormSubmit} type="button" className="z-20 fontPoppins text-2xl font-bold px-3 py-3  md:w-72 w-full rounded-md bg-red-600 ">
                                            SUBMIT
                                        </button>
                                    </div>
                                </form>
