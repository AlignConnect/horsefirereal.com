...
import { userInterSectionoberver } from './Observer';
...
const CustomerReview = () => {
    ...
    const { interSectionObserver, ref } = userInterSectionoberver();
    ...
    useEffect(() => {
        ...
        if (interSectionObserver && numberOfitemsShown < reviewsData.length) {
            console.log(numberOfitemsShown,reviewsData.length)
            showMore()
        }
                                ...
                                <div className="col-span-8 sm:col-span-6">
                                    ...
                                    <div className="flex items-center sm:pl-3 pl-2 sm:py-3 py-1">
                                        {iconsArray.map((_, index) => (
                                            <MdOutlineStarPurple500 key={index} className="text-yellow-500 text-lg sm:text-2xl" />
                                        ))}
                                        <span className="text-gray-700 sm:text-lg text-md pl-2">| 4.3 out of 5</span>
                                    </div>

                                    <div className="fontJosefin  bg-red-600 sm:text-lg px-3 text-sm text-center sm:pt-2 pb-1  w-[90%] mx-auto sm:mx-0 text-white font-bold sm:rounded-xl">
                                        LOVED BY CUSTOMER'S
                                    </div>
